...
d3.csv("India_Data_2.csv", function(error, data) {
  ...
  var path = SeriesLine.append("path")
    ...
    var sliderTray = slider.append("svg")
    .attr("class", "slider-tray");

    var sliderHandle = slider.append("svg")
    .attr("class", "slider-handle");

    sliderHandle.append("svg")
    .attr("class", "slider-handle-icon");

    slider.call(d3.behavior.drag()
    .on("dragstart", function() {
      dispatch.sliderChange(xD.invert(d3.mouse(sliderTray.node())[0]));
    ...
    sliderHandle.style("left", xD(value) + "px")
    });
